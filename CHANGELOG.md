...
- **Relocatable runtimes load their libraries inside the native sandbox.** An executable under `<prefix>/bin` gets read/execute on the sibling `<prefix>/lib` and `<prefix>/libexec`, so a conda/miniforge `node` no longer aborts on a missing `@rpath` dylib during a confined build.
