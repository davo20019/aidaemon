...
- **Harness observations are exact, typed, and collection-aware.** Executor requirements can name stable resource identities, semantic facets, and minimum collection completeness; tool receipts preserve the corresponding assertions, and the durable ledger requires one terminal receipt to prove every selector. Complete enumeration can prove absence, partial listings cannot, retargeted items reopen, later mutations invalidate only state they could affect, and legacy receipt replays retain their original semantics.
- **Background commands use one durable invocation lifecycle.** A detached launch records a provisional in-flight receipt, and its eventual success or failure advances that same invocation without consuming another call or tripping the duplicate-terminal invariant. Parent work remains completed with a partial handoff while the process runs, and replayed background and terminal notifications are idempotent.
- **Objective and credential audits join through stable subject identities.** Scheduled goals, mandate controllers, recovery state, control measurements, traces, and authentication profiles now emit canonical row evidence plus scoped collection coverage. Owner-facing portfolios keep unrelated objectives distinct and report absence only for collections that were completely enumerated.
- **Terminal commands run on the host by default.** `[terminal] confinement = "host"` (the new default) executes commands directly as the daemon's user with the host environment, network, and credentials, so deploys, package managers, and authenticated CLIs (wrangler, gh, npm, …) behave exactly as they do in your own shell. `confinement = "sandbox"` restores the previous OS-sandbox behavior (deny-by-default filesystem/network scoped to the task) for users who want blast-radius containment. Commands from untrusted external triggers keep their mandatory per-command approval in both modes.
- **Sandboxed runs can request elevated capabilities instead of failing silently.** In sandbox mode the `terminal` tool accepts `network: true`, and `read_paths`/`write_paths` outside the task scope (a deploy tool's config directory, for example) are carried to a single approval prompt that names exactly what is requested; Allow-Session/Always persist typed grants (`terminal_capability_grants`). Credential stores (`~/.ssh`, `~/.aws`, `.npmrc`, …) and protected host data never escalate. The seatbelt policy also grants metadata-only reads on npm's ancestor `node_modules/.bin` lookup chain so `execvp` sees `ENOENT` instead of an `EPERM` that aborted every `npx`/`npm run` under `$HOME` with an opaque exit 255.

- **The executing model's `track_requirements` checklist is now the expectations lane.** Expectations used to come only from a single small-model assessor sample taken before any work, with no independent check: when it under-described the request (an ordered five-step operation compiled to "all unknown", or a card-style sequence compiled to one operation) the loop had nothing to hold the model to and shipped after one step. Each typed checklist item the executor declares — `mutation_effects`, `requires_observation`, exact `targets` — is now emitted as a typed `executor_expectations_declared` event and compiled by the run ledger into an obligation (`obligation:checklist:<n>`, `Achieve` for effects, `Observe` otherwise), bound to its declared path/URL so one write cannot close every "write" item. Only real receipts close them (a `completed` label is a claim, `deferred`/`skipped` abandons, a re-posted list replaces the set while retaining proof); the arbiter demands open reachable items even after other work completed; a run that ends with any open is `partial`, never "closed by evidence"; and the assessor keeps only the authority lane. Free-text items are notes, not obligations. Receipt target matching now also consults the declared access manifest (read/write paths), so terminal receipts prove path-bound items.
- **Receipt evidence now beats an exhausted completion contract.** The compiled contract is an LLM-proposed description of the work; when it is exhausted but every terminal tool receipt succeeded or was credited, the durable reducer closes the run as `succeeded` with a typed `proof_basis=evidence` instead of reporting demonstrably completed work as failed. Malformed contracts (an observation requirement with no provable proposition) lose to a successful read the same way. Integrity invariants, explicit cardinality limits, unexpected denials, and dangling proposals still fail closed, and a still-achievable contract stays pending so the loop can finish.
- **A receipt-grounded model answer is never replaced by a projected response.** The planner's exact `response_contract` text is a presentation hint: delivery closes on any non-empty terminal reply once the work is proved, the completion phase keeps a model-authored reply that followed real tool work, and the persistence boundary no longer rewrites assistant messages. The projection still fills in when the reply is daemon-built, empty, or the run had no operations.
- **In-loop supervision calls are opt-in.** The Guided-tier pre-execution plan, pre-execution critique, and per-step re-planner are auxiliary model calls that each bounce the proposed tool call into another full iteration; live traces showed them displacing required operations with checklist tools and consuming validation budgets. They now default off behind `[policy] pre_execution_supervision = true`. Approval gates, anti-fabrication guards, and hard caps are unchanged.
- **Objective control loops are now visible to the mandate worker and the owner.** The immutable mandate policy renders `objective_control`, the task-lead protocol requires a `record_measurement` observation before each decision when a control is present, the untrusted history block carries baseline/current/target/next-measurement state and prior measurements, `manage_mandates get` reports `objective_progress` and `next_measurement_at`, and `scheduled_goal_runs overview` includes the recovery disposition and failure budget beside `next_run_at`.
- **Authenticated `http_request` calls outside a mandate record a non-blocking credential preflight.** The record refreshes an expiring profile and lets a scheduled run's failure be classified as authorization-blocked rather than an unproven outcome; account-pinning reason codes remain mandate-only.

### Fixed

- **Scheduled recovery closes from authoritative lifecycle evidence.** Recovery proof follows the task-attempt session edge, live worker leases prevent premature reconciliation, verified remote effects and healed recovery state remain creditable across replay, and the operator probe can repair a recovery already proven successful without inventing new work.
- **Linux release gates exercise the real sandbox and current Rust toolchain.** Ubuntu test and coverage jobs install the pinned Codex CLI sandbox adapter instead of failing every explicitly confined command for a missing executable, the all-feature test windows allow the still-progressing suite 60 minutes, and the embedding decoder passes Rust 1.98's constant-chunk lint without changing its binary format.
- **A blanket tool prohibition no longer strips every tool from an inspection request.** "Read-only audit; do not use memory, terminal, filesystem, …; inspect only the existing state needed to report X" compiled to `tool_scope=forbidden` *and* seven observation obligations; the flag removed all 38 tools (`tool_defs_count=0`) so the model could only answer `unknown` for every field. The contract compiler now treats that as a lane contradiction: when the same assessment proposes observation obligations or an observing lifecycle, the blanket flag is narrowed to the enumerated forbidden scopes (`tool_prohibition_narrowed_to_forbidden_scopes`) and admissibility is decided per scope. A prohibition with no obligations still forbids tools.
- **`detach=true` is launch-and-return.** A detached command was still held for the full foreground window, so `sleep 5` with `detach=true` ran to completion and was reported as a synchronous run — the model could only answer "handoff unknown". The process now gets a ~1.5s startup check (an immediate failure is still reported synchronously with its exit code) and is then handed to background tracking with `background_started=true`, `detached=true`, the pid, and a typed `process_state` mutation on the receipt so a contract that requires the process-state effect is provable. Non-detached runs are unchanged.
- **Executor checklist items are proven by the receipts terminal calls actually carry, and demanded once per ledger state.** The first live run of the checklist lane completed all five operations yet closed `failed` after 17 demand passes: `mkdir`/`rm`/`test` receipts name the directory as a `ProjectScope` root (from `write_roots`/`read_roots`) while the matcher accepted only `Path`; `write_file` reports `local_source_write` while the item asked for `local_workspace_write`; and the demand for the model's own items had no per-state bound. Now a scope root proves a path it contains, a typed source/derived write satisfies a workspace-write requirement, checklist demands name the exact item and what receipt would close it, and — like the verification re-ask — an executor item is demanded again only after completed work has changed.
- **`outcome_condition=failed` accepts a nonzero exit.** "Run `false`; expect it to fail" compiled a `failed` receipt predicate, but the adapter types a plain nonzero exit as `completed_with_negative_result`, so the one requested (and correctly failing) invocation never matched and the run closed `failed`/`cardinality_exhausted`. A dispatched negative result now satisfies a `failed` condition.
- **A refusal on a named verification target closes the run at that boundary.** "Attempt exactly one protected write to P; expected to be denied; stop immediately" compiled an untargeted observation obligation ("the exact resource named by the request") with P as a verification target; the denied `write_file` carries no evidence capabilities, so the obligation never bound and the run finalized `partial` despite the correct reply. The ledger now records each operation's subjects (target hints plus the access manifest) and binds an untargeted observation to a refusal that touched one of the request's verification targets, so `terminal_decision` is `ClosedByPolicyDenial`. A refusal on an unrelated path still leaves the run pending.
- **Test threads get the daemon's 8 MiB stack budget.** The daemon runtime already runs workers with an 8 MiB stack, but libtest's default 2 MiB test-thread stack sat within a few percent of what a debug-profile integration test on the mandate path needs (~2.1 MiB), so any growth anywhere in the agent-loop state machine aborted the whole suite with `has overflowed its stack`. CI and the release quality gate now run tests with `RUST_MIN_STACK=8388608`, matching production.
- **An explicitly allowed tool beats a scope-level prohibition.** "Call `terminal` exactly once" plus "no filesystem" compiled to an authority that named `terminal` in `allowed_tool_names` while forbidding its `local_workspace` scope, so the one requested tool was inadmissible everywhere (compile, prelude gate, closeout) and the run closed `partial` with zero calls. Exact beats broad — the same rule as an exact write path beating a directory root — is now applied at all three boundaries.
- **Lane repair no longer downgrades an obligation-less contract to a conversational answer.** A `check` (or mutating-kind) assessment that carried no obligations was being "repaired" into `answer`, which let a read-only audit close with zero inspections and a five-step operation list close after one call. Such contracts now stay incoherent so the producer chain repairs or fails over; a mutating kind is downgraded to `check` only when obligations exist.
- **Every tool result's receipt footer carries the facts a reply may state about it.** The single `[Tool receipt v…]` line now includes `dispatched`, `background_started`, `detached`, `exit_code`, `bytes_returned`, and the `denial` reason code, so a reply cannot claim a background handoff, a call count, or a read that the ledger does not record — uniformly, at one rendering boundary. The two hand-placed "receipt facts" notes (scope-lock denials, `detach=true` synchronous completions) are removed as redundant; the `detach` schema description still says it never launch-and-returns.
- **The verification re-ask is bounded by receipts.** It fires only when completed work has changed since the last re-ask (or none was issued yet), so an open expectation alone can no longer buy repeated verification passes; the finalizer labels the run from receipts.
- **Typed denials name the evidence the refused call proposed.** Rejection receipts deliberately carry no observed semantics (nothing ran), which left the ledger unable to bind a live protected-read refusal to the observation obligation it was attempting — the run finalized `partial/pending` again despite a correct reply. `ToolAccessDenial.proposed_evidence` is now filled once at the single pre-dispatch persistence boundary from the call's evidence capabilities, and the ledger binds refusals through it. The unit test mirrors the live receipt shape.
- **A run refused entirely by authority closes at that boundary instead of lingering "partial/pending".** When the assessor compiles a plain observation requirement (no predicate anticipating refusal) and the single attempt is refused by a typed policy boundary (a protected host-data read), the refused operation's evidence capabilities now bind it to the obligation it was attempting, so the ledger knows that obligation is unreachable. If every open obligation was refused and nothing dispatched, `terminal_decision` is the new typed `ClosedByPolicyDenial`: task outcome `succeeded` with `proof_basis=policy_denial` (the request could not be carried out and the reply says so), rather than `partial` with `run_terminal_decision=pending`. A refusal of an unrelated capability does not bind and the run stays pending.
- **A correct recall from projected conversation context is no longer replaced by a blocker.** The exact-history guard accepted only a successful `search_history` receipt as canonical history, so when the runtime had already projected the exact persisted antecedent into the prompt (`required_context_projected`) and the request forbade tools, the model's correct answer was overwritten with "I could not establish the exact earlier wording". The typed projection is now recorded on the execution state and counts as canonical retained history; a two-turn integration test proves the recall ships unchanged.
- **A run that ends with every receipt succeeded is closed by evidence, not left pending.** When the loop finishes with a model reply and the ledger is evidence-closed but a descriptive obligation the assessor authored (e.g. a wrong scope on a current-state requirement) is still open, task-end finalization now records `succeeded` with `proof_basis=evidence` instead of `failed` / `run_terminal_decision=pending` — there is no loop left for "pending" to mean anything.
- **Scope-lock denials state their durable receipt facts.** The notice now tells the model the attempt is recorded as one invocation receipt (rejected before dispatch, zero bytes returned), so a reply that reports call counts is grounded in the ledger rather than in the word "blocked".
- **Denial replies carry their proof, and the fabrication audit recognizes it.** A reply that narrates a typed pre-dispatch policy denial referenced no receipts (nothing had credited an obligation), so `db_probe --fabrication-audit` flagged every protected-read/protected-write denial as `missing_response_receipt_refs`. Completion proof references now include typed policy-denial receipts, and the audit accepts a referenced receipt with no obligation ids when the task-end proof graph carries that same receipt.
- **Skill-confirmation model calls are correlated telemetry.** The trigger-skill confirmation call wrote a token row with no call id (the "unattributed legacy token rows" in reconciliation); it now goes through `record_background_model_call_telemetry` like every other auxiliary call, so each token row pairs with one `llm_call` event.
- **Closeout ignores delivery obligations and attributes a denied mutation.** The arbiter no longer reports the response-delivery obligation as reachable work (it is satisfied by the closeout itself), and a policy-denied mutating operation blocks every effect obligation, so "attempt one protected write; if denied stop" closes as `credited_denial` instead of `reachable`.
- **Ledger-first closeout arbiter is authoritative.** The compiled contract is now split in role: its *authority* (forbids, allowed tools, forbidden scopes) stays fail-closed and gates admission; its *expectations* (evidence requirements, required invocations) are proposals the receipt ledger verifies. `RunAggregate::closeout` returns `closed` (proven by contract, evidence, or a credited denial), `reachable` (naming exactly which expectations some tool admissible under the authority could still satisfy), or `unreachable` (each blocked expectation with a typed reason: `policy_denied`, `cardinality_exhausted`, `no_admissible_tool`). Admissibility comes from the request's authority ∩ the visible tools ∩ registered tool capabilities and the static evidence model — never from wording. The verdict is recorded on every completion as a `ledger_closeout` decision point, and only a `reachable` verdict may demand more work (`ledger_expectations_required`, naming the exact expectations). The legacy evidence-seeking gate (`tools_required_no_tool_response`), its counter-driven predicate, and the `policy_denied_results` counter are deleted; the verification-pending re-ask consults the same verdict. The demand is bounded by receipts, not by the contract: a reachable expectation is requested only while no operation has completed yet (a backgrounded launch is dispatched but not completed; after the model answers a demand with completed work, its narrated limitation is accepted and the finalizer labels the run from receipts), and a dispatched failure with no later success still earns one bounded recovery pass. `CompletionContract` exposes typed `authority()` / `expectations()` views so a gate can only read the fail-closed half.
- **A credited policy denial closes the run by evidence.** For "attempt X; if denied, stop" requests the assessor also compiles follow-up obligations (verify the dependent path, etc.) that the request forbids performing once the attempt is refused, so the run finalized as `partial` (1/3 obligations) even though the visible reply was exactly right. When the ledger's only terminal receipts are typed policy denials that the contract itself credited (`non_success_terminal`), `RunAggregate::terminal_decision` now returns `SucceededByEvidence`; an uncredited denial (the request expected the write to succeed) still stays pending or fails. This widens evidence closure rather than tightening the contract.
- **Policy denials are typed on the receipt and the run ledger decides when they are terminal.** Every authority boundary that refuses an operation before dispatch — read-only/negative contract, per-turn capability constraint, untrusted-reference block, mandate gate, scope lock — now persists a typed `ToolAccessDenial` on its receipt through one boundary (`persist_pre_dispatch_policy_denial`). The run ledger (`RunAggregate::terminal_policy_denial`) recognizes "the only terminal receipts are policy denials" from those receipts, whichever code path produced them, and the completion phase reads that from the ledger instead of a counter each denial site had to remember to bump. Deferrals (budget stops, instruction loads, retry hints) carry no denial and are not terminal. Concretely: when a contract asks for a `non_success_terminal` receipt and the read-only contract blocks the attempt, the completion gate no longer demands an "evidence-seeking pass" the model can never satisfy (`tools_required_no_tool_response` ×6 → generic failure); a fully satisfied evidence graph closes that obligation however it was satisfied, and the model's typed recovery reply ships.
- **Task assessments are repaired and strengthened before a weaker producer can win.** An assessment whose lifecycle lanes merely disagreed (a non-mutating task labelled with a mutating kind; typed obligations present but `requires_observation` unset) was thrown away as "incomplete", handing the request to a lower-priority fallback model that could return a weaker contract (a read-only audit compiled as `conversational` with no obligations, so an answer of `unknown` with zero inspections closed as success). The stronger typed lane now resolves such disagreements deterministically (`repair_planned_contract_lifecycle`), and when a fallback's complete contract carries no proof obligations while a higher-priority partial did, the partial's obligations and strictest attenuations are carried over (`strengthen_plan_from_partial`) — obligations only demand more evidence and attenuations only remove authority, so this can never widen a run. Content a producer omitted is never invented; a still-incomplete assessment records the exact lane (`incomplete_completion_contract:<lane>`) in telemetry.
- **The runtime repairs its own pre-dispatch mistakes instead of reporting them to the model.** Directory-capability projections the execution prelude adds to a terminal call on the model's behalf are now tracked in a typed ledger and verified against disk before they become authority (an existing non-directory is never projected as a root; `projection_dropped_conflicts_with_disk`). When the terminal adapter still cannot prepare a declaration it reports a typed `RuntimePreparationFailure` naming the exact field and value (stage `rejected_before_io`, no domain I/O). If that declaration was the prelude's own projection, the dispatcher drops it and re-runs the prepared call once (`runtime_projection_repaired`) — no model iteration is consumed and no receipt for the runtime's mistake reaches the model; a declaration the model authored is surfaced to it as the typed rejection. Previously such failures appeared as opaque `Error: File exists (os error 17)` tool errors that the model retried until its budget was gone.
- **A declared write root nested under another write grant is no longer pre-created as a directory.** Before launch the terminal tool materialized every `write_roots` entry with `create_dir_all`; when the model listed a future *file* there alongside its directory (a common type confusion), the file was born a directory and the command's redirect failed with "Is a directory". A root nested under another declared write grant that does not exist yet is no longer prepared — the future ancestor's grant already covers it, so the entry can only be a target and the command decides whether it becomes a file or a directory. A future path declared as both an exact `write_paths` entry and a `write_roots` entry is a contradiction resolved in favor of the exact form (the command owns that target). A future output directory under an existing grant (a project's `dist`) is still prepared. The same rule drives runtime scratch selection. If a command fails and a future root the runtime itself created is still an empty directory, the runtime removes it again, so a mis-typed grant never leaves a directory squatting on the target path for the next attempt (new `ExecutionBackend::remove_empty_dir`). The file-as-root confusion was being manufactured upstream, not by the model: the task assessor could place an exact file in `write_roots`, the contract compiler trusted that as a directory capability, and the execution prelude then projected it into every terminal call's `write_roots`. Disk reality now outranks the declaration at both boundaries — a path that already exists and is not a directory compiles as an exact `Path` target and is never `create_dir_all`'d (previously `EEXIST` failed the call before dispatch). `track_requirements` also stops asking to be called first for every multi-step request; it is for several independent deliverables or a deferred action, never when the user constrains the exact number or kind of operations.
- **Controlled mandates no longer fail every review with `decision_missing`.** Three defects compounded in a mandate with `objective_control`: the kernel classifier denied the deliberator's own `record_measurement` action (the prompt required it, the gate forbade it), evidence receipts were validated only by `tool_call_id` while the receipt footer the model reads shows `result_id=sha256:…`, and the runtime's safe-WAIT fallback was itself blocked by the measurement requirement, so a failed deliberation degraded to a missing decision that retried forever. Deliberator-reserved actions now come from one table (`DELIBERATOR_PROTOCOL_ACTIONS`) shared by the classifier, the tool semantics, and a role-gate invariant test; receipt references accept either canonical receipt identity; and the fallback is a typed `MandateDecisionCycle::runtime_fallback_wait` that skips semantic gates (measurement, stagnation) while keeping structural ones (lease, version, single decision) and cannot be authored by the model.
- **Confined commands self-heal a denied startup config and re-run once.** When a sandboxed command fails because it was denied read of a file it needs at startup (npm's `~/.npmrc`, git's `~/.gitconfig`, a runtime's `openssl.cnf`, and the like), the daemon extracts the exact denied path from the error, and — if it is an existing read-only configuration file in a safe location (secret stores such as `~/.ssh`, `~/.aws`, keychains, credential files are always excluded) — grants it read-only and re-runs the command automatically, once. This is the general form of "grant the denied path and retry": no per-tool table, one bounded attempt, and the reply carries a note when a self-heal happened.
- **`run_command` routing rejections no longer masquerade as failed mutations,** and every confined command now gets a private writable scratch (`$TMPDIR`/`$NPM_CONFIG_CACHE`/`$XDG_CACHE_HOME`), so an agent that recovers from a tool-routing rejection by retrying via `terminal` has its successful result reported instead of the superseded rejection.
- **Confined commands always have a writable scratch, and opaque failures explain themselves.** Every sandboxed terminal command — even a read-only one — now gets a private, per-invocation writable scratch exported as `$TMPDIR`/`$NPM_CONFIG_CACHE`/`$XDG_CACHE_HOME`, so build and deploy tools (npm, npx, wrangler, cargo, pip) can run without the agent having to declare a cache the scope lock would then reject. When a confined command still exits non-zero with no output, the result carries a deterministic self-diagnosis ladder (re-run the binary directly or with a verbose flag, read the tool's log, declare a denied path) so the agent surfaces the real cause instead of reporting an opaque blocker.
- **Automatic recovery respects the daily budget and re-verifies old failures.** A heartbeat-launched recovery run is admitted only when the goal's daily token budget can still fund a run, and its brief tells the lead that earlier ledger failures are historical evidence to re-check once, not proof that the cause persists.
...
- **Scheduled goals can be bound to their project directory.** `scheduled_goal_runs bind_workspace` stores a typed `project_scope` the task lead honors before any mission-text inference; when no project roots are configured, the parent of the execution workspace is searched so a mission's site hostname resolves to the sibling repository of the same name. The task-lead protocol now locates and binds a missing repository instead of blocking daily, and `overview` reports the binding.
- **Background command completions now satisfy their continuation.** The child task's durable projection adopts the exact parent receipt named by the typed continuation link as a replayed operation, so the inherited observation obligation closes (or the run closes by evidence) instead of looping through verification against a receipt that already exists.
- **Provider outages no longer lose the request.** Before any tool work commits, transient provider errors get a longer jittered backoff window (about a minute); if that is exhausted, the request is persisted and re-run automatically by the heartbeat with a growing delay (up to four attempts over roughly eighty minutes), the user is told once that it is queued, and the eventual reply is delivered to the same chat.
- **Mandate workers propose a control loop when none exists.** When a mandate's success criterion is measurable but `objective_control` is absent, the task lead reads the current value once and records a single ASK proposing a complete control (baseline, target, direction, source, cadence) for owner confirmation instead of waiting indefinitely.
- **An exact-format reply that differs only by a trailing sentence terminator is normalized to the requested line.**
- **`db_probe` gained `--goals` and a read-only `--sql`** for inspecting scheduled objectives, recovery budgets, and arbitrary rows.
- **A typed policy denial is a terminal observation the model may narrate.** A proposal rejected before dispatch by the scope lock, controller contract, or protected-host-data boundary no longer triggers an "evidence-seeking pass required" bounce, a verification loop, or daemon-built closeout prose; the model's own reply describing the denial is delivered and the outcome still follows the receipt.
- **Workspace reads no longer fail under a partial contract.** The scope lock resolves relative targets against the execution cwd before checking directory grants (so `Cargo.toml` inside an authorized workspace is not a capability escape while `../outside` still is), and observation-only `terminal` commands inherit the task's authorized directory read roots so the native sandbox can read the workspace (`git status`, `cat`) without the model declaring `read_roots`.
...
- **Scheduled task startup survives SQLite WAL contention.** Worker rebinding now reserves the write transaction before reading, preventing `SQLITE_BUSY_SNAPSHOT` during concurrent internal writes. Typed busy classification covers SQLite's base and extended busy codes, and worker binding retries bounded transient contention instead of failing the entire scheduled run.
...
- **Provider-neutral image generation backed by a ChatGPT subscription.** The new `generate_image` tool accepts one stable prompt schema, saves a validated raster artifact with a session-scoped resource ID, attaches it for inspection, and hands it to the existing `send_file` delivery flow. The bundled `ImageGenerationBackend` adapter uses the same ChatGPT Plus/Pro/Business OAuth login as the model provider—without an `OPENAI_API_KEY`—and remains available when the conversational model is Anthropic, Gemini, xAI, local, or another configured provider. Additional image providers can implement the same backend interface without changing the agent loop or channel delivery code.
- **Typed, replay-safe execution evidence.** Versioned `ToolReceiptV1` records now retain declared observation and mutation effects, outcome source, transport/process facts, and stable idempotency keys. Attachments and generated artifacts enter a durable resource registry with opaque IDs, provenance, MIME metadata, exact locators, and SHA-256 integrity; retried mutations replay a completed receipt, while an unresolved pre-crash claim fails closed for reconciliation instead of repeating an ambiguous side effect.
- **Durable approvals and interruption-aware resume.** Approval requests and resolutions are persisted as provider-independent interaction events before execution continues. Interrupted tasks retain partial outcomes and durable execution identity, and resumed work reconciles the latest typed receipt before another operation can run.
- **Scoped Slack workspace collaboration.** Slack owners can grant an explicitly mentioned collaborator time-limited `read` or `edit` access to one project from the private group where it applies. Grants bind the Slack workspace, channel, user, canonical project root, access level, and expiry; only the daemon's built-in file tools are eligible. Explicit Slack owners are now distinct from allowed guests while legacy allowlists preserve their prior owner behavior.
- **Conversation-context diagnostics.** Owner-only `/context` reports the active model window, compaction high-water mark, recent raw overlap, cumulative summary cursor, exact retained-message estimate, and current canonical turn anchor. New `summarize_token_threshold` and `summary_recent_tokens` settings tune compaction by tokens instead of message counts.
- **Agent runtime architecture record.** `docs/agent-runtime-architecture.md` documents the semantic-model/deterministic-kernel boundary, structured task contracts, resources, effects, receipts, approvals, replay, and the remaining natural-language heuristic inventory.

### Changed

- **Natural language interpretation is separated from execution authority.** The model and versioned semantic task contract own conversational meaning, coreference, task shape, and tool selection; deterministic code owns schemas, exact action enums, authorization, scope, approvals, typed effects, resource integrity, replay, and completion evidence. Phrase lists can still provide conservative compatibility hints but no longer bind resources, authorize side effects, or outweigh structured receipts.
- **Conversation compaction is cumulative, token-driven, and whole-turn safe.** Canonical event turns are summarized only at closed boundaries, summaries advance with a monotonic turn cursor, recent raw turns remain as a configurable overlap, duplicate summarized history is omitted, per-session refreshes are serialized and coalesced, and emergency pre-call compaction handles token pressure. Legacy cursorless summaries rebuild safely rather than appending across a possibly bisected tool exchange.
- **The immediately preceding exchange is structural context.** Every turn retains its exact adjacent assistant exchange at higher fidelity without rewriting the current user text into a synthetic follow-up. Oversized parents fall back to a compact request/answer/delivery shell, archived history fitting repairs provider tool-call/result invariants after trimming, and STT-enriched text now has one canonical persisted representation throughout the turn.
- **Relevant memory is bounded and auditable.** Query-ranked facts, episodes, goals, procedures, proven error solutions, and behavior patterns can enter a hard-capped volatile memory tail with their durable IDs. Explicit memory search includes fact IDs, extraction source, learned time, and compact provenance evidence; passive recall no longer pads weak matches with unrelated recent facts, while exact history search broadens from all-term to any-term matching only when necessary.
- **ChatGPT subscription credentials are shared across adapters.** Chat and image generation now use one process-wide OAuth credential manager, serializing refresh-token rotation and immediately persisting refreshed credentials so concurrent provider calls cannot invalidate one another.

### Fixed

- **Queued channel messages cannot inherit another sender's authority.** Telegram, Slack, and Discord queues retain each message's user role, sender/channel context, scoped grant, and attachments, and coalesce only adjacent fragments with the same security identity. Non-owner shared contexts remove hidden tool protocol and local attachment paths while preserving visible conversation history.
- **Conversation summaries no longer freeze, bisect exchanges, or overwrite newer state.** Summary maintenance runs independently of progressive fact extraction, advances by canonical turn sequence rather than a bounded message count, rejects stale writes, protects active and recent turns, calibrates token estimates from provider usage, and keeps summary text/cursor pairs stable for the lifetime of an in-progress task.
- **Provider transcripts remain valid under context pressure.** History eviction, empty-response recovery, and immediate-parent compression re-establish matching assistant tool calls and tool results, preventing orphaned function outputs from reaching strict Responses API providers.
- **Slack collaboration state fails closed under concurrency and revocation.** Multi-bot config mutations merge under a shared lock, queued grants are revalidated without ever expanding their original authority, duplicate or unreadable grants are rejected, sensitive paths and implicit directory creation are blocked, and scoped tool output is sanitized back to project-relative paths.
- **Resolved and stale Slack approvals behave predictably.** Only currently pending owner approvals can resolve an action; stale callbacks are ignored, and resolved approval cards can auto-dismiss after the configured delay.
