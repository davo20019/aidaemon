...
- **`detach=true` completions state their receipt facts.** `detach` keeps a long-lived process alive past task end; it never launch-and-returns. A command that finishes within the foreground window now appends a receipt-facts line (`background_started=false`, `detached=false`) so the reply cannot claim a background handoff the ledger does not record; the schema description says the same.
