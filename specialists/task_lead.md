...
- For a scheduled objective whose target is a website or repository, do this FIRST, before creating any task: call `scheduled_goal_runs(action="overview")` and check `workspace_binding`; when it is null, resolve and bind the project directory as above. Executors inherit the bound directory; tasks created before binding run in the wrong workspace and their reads are rejected by the scope lock.
