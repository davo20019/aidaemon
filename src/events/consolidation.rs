...
impl Consolidator {
    pub fn new(event_store: Arc<EventStore>, plan_store: Arc<PlanStore>, pool: sqlx::SqlitePool) -> Self {
        Self { event_store, plan_store, pool }
    }
    ...
    pub async fn consolidate_session(&self, session_id: &str) -> anyhow::Result<ConsolidationResult> {
        ...
        if let Some(episode) = self.create_episode_summary(session_id, &events).await {
            if let Err(e) = self.save_episode(&episode).await {
                warn!("Failed to save episode: {}", e);
            } else {
    ...
    async fn extract_procedures(&self, events: &[Event]) -> Vec<Procedure> {
        ...
        for (task_id, task_evts) in task_events {
            ...
            if steps.len() >= 2 {
                let proc = Procedure {
                    ...
                    trigger_pattern: start_data.description,
                    steps,
                    success_count: 1,
                    failure_count: 0,
    ...
    async fn extract_error_solutions(&self, events: &[Event]) -> Vec<ErrorSolution> {
        ...
        while i < events.len() {
            ...
            if event.event_type == EventType::Error {
                if let Ok(error_data) = event.parse_data::<ErrorData>() {
                    ...
                    for j in (i + 1)..events.len().min(i + 10) {
    ...
    async fn extract_expertise_updates(&self, events: &[Event]) -> Vec<(String, bool)> {
    ...
    async fn save_procedure(&self, proc: &Procedure) -> anyhow::Result<()> {
    ...
    async fn save_error_solution(&self, es: &ErrorSolution) -> anyhow::Result<()> {
    ...
    async fn update_expertise(&self, domain: &str, success: bool) -> anyhow::Result<()> {
        let now = Utc::now().to_rfc3339();
