...
/// Sanitize structural protocol artifacts before text is shown to end users.
///
/// Ordinary capability names are valid user-facing prose. They are not a
/// protocol boundary and must not be removed based on a lexical list: doing so
/// corrupts explanations, hypothetical statements, and exact-response tasks.
pub fn sanitize_user_facing_reply(reply: &str) -> String {
    ...
    strip_meta_instructions(&identity_cleaned)
}
...
///
/// Ordinary capability names are deliberately preserved; only structural
/// control/protocol forms are removed.
...
mod tests {
    ...
    #[test]
    fn user_facing_sanitizer_preserves_ordinary_capability_prose() {
        let input = "I would use terminal to run commands.";
        assert_eq!(sanitize_user_facing_reply(input), input);
    }
