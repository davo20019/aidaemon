...
impl Tool for ManageMemoriesTool {
    ...
    fn description(&self) -> &str {
        "List/search/forget memories, and create/list/configure scheduled goals. Use scheduled_goal_runs to update, trigger, or inspect executions."
    }
    ...
    async fn call(&self, arguments: &str) -> anyhow::Result<String> {
        ...
        match args.action.as_str() {
            ...
            "trigger_now" => {
                ...
                // Keep this compatibility action on the same implementation as
                // `scheduled_goal_runs(run_now)`. The former duplicate path did
                // not copy goal context or establish a fresh rooted goal run,
                // which could silently drop updated instructions and attach a
                // manual task to stale ownership state.
                crate::tools::ScheduledGoalRunsTool::new(self.state.clone())
...
mod tests {
    ...
    #[tokio::test]
    async fn trigger_now_uses_run_aware_path_and_preserves_goal_context() {
        let state = setup_state().await;
        let tool = ManageMemoriesTool::new(state.clone());

        let mut goal = Goal::new_continuous(
            "Publish one researched daily insight",
            "user-session",
            Some(1000),
            Some(5000),
        );
        ...
        state.create_goal(&goal).await.unwrap();
        add_schedule(&state, &goal.id, "0 9 * * *").await;
